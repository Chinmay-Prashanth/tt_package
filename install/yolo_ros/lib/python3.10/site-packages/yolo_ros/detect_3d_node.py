...
class Detect3DNode(LifecycleNode):
    ...
    def convert_bb_to_3d(
        ...
        if detection.mask.data:
            ...
            # Clamp the center coordinates to be within the image bounds
            clamped_center_x = int(min(max(center_x, 0), depth_image.shape[1] - 1))
            clamped_center_y = int(min(max(center_y, 0), depth_image.shape[0] - 1))
            bb_center_z_coord = (
                depth_image[clamped_center_y][clamped_center_x] / self.depth_image_units_divisor
            )
        ...
        # project from image to world space using intrinsic parameters
        k = depth_info.k
        ...
        # create and return 3D bounding box message
        msg = BoundingBox3D()
